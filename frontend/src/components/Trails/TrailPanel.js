...
const TrailPanel = ({ trails, isPanelOpen, togglePanel, zoomToTrail }) => (
  <>
    <button
      onClick={togglePanel}
          ...
          <ul className="space-y-4">
            {trails.map((trail) => {
              ...
              return (
                <li
                  key={trail.id}
                  className="p-4 bg-white shadow-md rounded-lg border border-gray-200 relative cursor-pointer"
                  onClick={() => zoomToTrail(trail)}
                >
                  <div>
